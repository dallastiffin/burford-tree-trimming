# Burford Tree Service — Website

Static site for **Burford Tree Service**, Burford, Ontario. No framework, no build server, no
monthly hosting bill.

```
Burford-Tree-Trimming-Website-Content.md   <- every word on the site (source of truth)
...
This town runs `HEADER_VARIANT = "tworow"` and `HERO_VARIANT = "centre-stack"`, set near the
...
**Copy** — edit `Burford-Tree-Trimming-Website-Content.md`, then `python build.py`.
...
The repo is set up for Cloudflare. The worker name is `burfordtreetrimming` and the
canonical host is `www.burfordtreetrimming.com`.
